...
class GuiConsultation():
    ...
    def __init__(self,idEspece,parent=None):
...
#        self.comboBox_typeanalyse.Fill(self.MyAnalyse.GetListTypeAnalyse,True)
        self.comboBox_typeanalyse.setModel(self.MyAnalyse.ListeTypesAnalyse)
        self.comboBox_typeanalyse.setModelColumn(self.MyAnalyse.ListeTypesAnalyse.fieldIndex('Libele'))
        ...
        self.toolButton_TypeConsultation.clicked.connect(self.OnEditTypeConsultation)
        ...
        self.toolButton_EditTypeAnalyse.clicked.connect(self.OnEditTypeAnalyse)
        ...
        self.toolButton_EditParametre.clicked.connect(self.OnEditParametre)
    ...
    def FillConsultation_Combo(self):
        ...
        self.comboBox_veterinaire.setModel(MyComboModel('GetConsultants()'))
        self.comboBox_Referant.setModel(MyComboModel('GetReferants()'))
        self.comboBox_consultType.setModel(MyComboModel('GetTypesConsultation()'))
        self.comboBox_PathologieDomaine.setModel(MyComboModel('GetDomaines()',u'Tous'))
#        self.comboBox_Pathologie.setModel(MyComboModel('SelectPathologies(%i,%i)'%(self.MyPathologie.idEspece,self.idDomaine),u'Néant'))
...
#        self.comboBox_Pathologie.Fill(self.MyPathologie.GetPathologies(0,u'Néant'))
    ...
    def OnEditTypeConsultation(self):
        id=self.comboBox_consultType.Getid()
        new=[0,'','',True,'']
...
#        model=MyTable('TypeConsultation',id)
    ...
    def OnDomaine(self):
        self.comboBox_Pathologie.setModel(MyComboModel('SelectPathologies(%i,%i)'%(self.MyPathologie.idEspece, self.comboBox_PathologieDomaine.Getid()),u'Néant'))
#        self.idDomaine=self.comboBox_PathologieDomaine.itemData(self.comboBox_PathologieDomaine.currentIndex(),QtCore.Qt.UserRole).toInt()[0]
#        self.comboBox_Pathologie.Fill(self.MyPathologie.GetPathologies(self.idDomaine.toInt()[0],u'Néant'))
...
#________________________________________________***  A N A L Y S E S ***_______________________________________________________
    def GuiAnalyse(self):
        ...
        if not self.MyAnalyse.isImage:
            ...
            self.tableView_Parametres.autoResize(1)
    ...
    def OnTypeAnalyse(self,index=None):
        self.idTypeAnalyse=self.MyAnalyse.GetIdTypeAnalyse(self.comboBox_typeanalyse.currentText())
        self.MyAnalyse.IsQuantitatif(self.comboBox_typeanalyse.currentText())
        if self.idTypeAnalyse is None:
            return
        self.GuiAnalyse()
        if self.MyAnalyse.idAnalyse==0:
            if not self.MyAnalyse.isImage:
                self.MyAnalyse.Resultats.SetParametres(self.idTypeAnalyse,self.idEspece)
    ...
    def OnEditTypeAnalyse(self): 
        self.idTypeAnalyse=self.MyAnalyse.GetIdTypeAnalyse(self.comboBox_typeanalyse.currentText())
        MyFormTypeAnalyse=FormTypeAnalyse(self.idTypeAnalyse)
        if MyFormTypeAnalyse.exec_():
            #TODO maj comboBox_typeanalyse
            pass
    ...
    def OnModelAnalyse(self,index=None):
        if not self.MyAnalyse.Resultats.isEmpty():
            if QtGui.QMessageBox.question(self,'OpenVet',u'Voulez-vous effacer les données présentes?',QtGui.QMessageBox.Yes| QtGui.QMessageBox.Default,QtGui.QMessageBox.No)==QtGui.QMessageBox.No:
                return
        idModele=self.comboBox_model.itemData(index,QtCore.Qt.UserRole).toInt()[0]
        if idModele>0:
            if not self.lineEdit_description.text().isEmpty():
                if QtGui.QMessageBox.question(self,'OpenVet',u'Voulez-vous assigner le nom de modèle à la description?',QtGui.QMessageBox.Yes| QtGui.QMessageBox.Default,QtGui.QMessageBox.No)==QtGui.QMessageBox.Yes:
    ...
    def OnEditModel(self):
        ...
        if flag==3:
            ...
            if choix==1:
                ...
                self.MyAnalyse.Resultats.Modeles.removeRows(self.comboBox_model.currentIndex())
                self.lineEdit_description.setText(QtCore.QString(''))
            ...
            if msgbox.exec_()==QtGui.QMessageBox.Yes:
                ...
                if MyModele.exec_():
                    ...
                    self.comboBox_model.setCurrentIndex(index)  
        #TODO: traiter les retours err=save 
    
    def OnEditParametre(self):
        index=self.comboBox_Parametre.currentIndex()
        idParametre=self.MyAnalyse.Resultats.Parametres.GetParametre(index)[8]
        self.MyParametre=Core_Analyse.Parametre(idParametre,self.MyAnalyse.Resultats.Parametres)
        MyFormParametre=FormParametreAnalyse(self.MyParametre)
        if MyFormParametre.exec_():
            self.MyAnalyse.Resultats.SetParametres(self.idTypeAnalyse,self.idEspece)
            self.comboBox_Parametre.setModel(self.MyAnalyse.Resultats.Parametres)
