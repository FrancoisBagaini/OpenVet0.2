...
# extraction substances actives
    ...
    substances = []
    for i in comp:
        r = re.findall(ur' \.* +[0-9]', i,re.UNICODE)
        if len(r) == 0:
            continue
        end = i.index(r[0])
        med = i[:end].strip()
...
def GetPosologie(chaine, substances):
    ...
    doses = []
    for i in substances:
        r = re.findall(ur'[0-9]*[ ,]?[0-9]+.+de ' + i[0] + ' par ', comp[start:],re.UNICODE)
        if len(r) > 0:
...
def GetPresentation(chaine):
    ...
    amm = False
    for i in comp:
        r = re.findall(ur'FR/.+[0-9]+ [0-9]{1,2}/[12][0-9]{3}', i,re.UNICODE)
        if len(r) == 1:
            amm = True
            continue
        else:
...
#        sols=re.findall(r'[0-9]+[ ,]?[0-9]*',chaine)
        chaine=re.sub(ur'[uU]ne?','1',chaine,re.UNICODE)
        sols = re.findall(ur'[0-9 ,]+', chaine,re.UNICODE)
        try:
...
def ExtractUnite(chaine):
    return re.sub(ur'[0-9, ]|une?','',chaine,re.UNICODE)
    

def Getunite(chaine):
        sols = re.findall(ur'([0-9 ,]+)(DL50|kBq|mEq|millions UI|million.*internationales|[a-z]+)', chaine,re.UNICODE)
...
def GetShortPresentation(chaine):
    first = re.findall(ur'(^[0-9 ]*\S+)', chaine,re.UNICODE)  # TOTEST r'(^[0-9 ]*\S[^\(\)]+)(?:\(s\))?[ -]'
    last = re.findall(ur'de ([0-9, ]+\S+$)', chaine,re.UNICODE)
    if len(first) == 0:
...
#             if words[0]=='60092590':
#                 print words
            longname=re.sub(ur' POUR PR.PARATIONS HOM.OPATHIQUES| BASE| ANHYDRE| SODIQUE','',words[3],re.UNICODE)
            shortname = re.sub(ur'(^\S+ATE D[E\'] ?)|( \(.+\))|(,.+$)|( \S*HYDRAT\S+$)', '', longname,re.UNICODE) 
            if longname.count(':'):
...
def ImportClasseTherapeutique(filin,filout,parent):
    ...
    while not 'INDEX . . .' in line:
        line=re.sub(ur'\. +','. ',line,re.UNICODE)
        sol=re.findall(ur'(([0-9]{1,2}\.)+)',line,re.UNICODE)
        if len(sol)>0:
            if sol[0][0]=='2.3.':
                print line
        sols=re.findall(ur'(([0-9]{1,2}\.)+ [^\.]+)',line,re.UNICODE)
        if len(sols)>0:
            newline=sols[0][0]
            if sols[0][0][-1:]=='\n':
            ...
            sols=re.findall(ur'\S[^\.]+ . . .',line,re.UNICODE)
            if len(sols)>0:
                if len(re.findall(ur'[a-z]+',sols[0],re.UNICODE))>0 and not header is None:
...
def HomogenMolecule(chaine,codec='latin-1'):
    chaine=re.sub(ur' \S+HYDRATÉE?','',chaine,re.UNICODE) #chaine=re.sub(ru' \S+HYDRATÉE?','',chaine.decode(codec),re.UNICODE)
    tmp=re.findall(ur'\(\S+ D[\'E]\)',chaine,re.UNICODE)
    if len(tmp)>0:
        chaine=re.sub(ur'\(\S+ D[\'E]\)','',chaine,re.UNICODE)
        esp=''
        if tmp[0][-2:-1]=='E':
            esp=' '
        chaine=tmp[0].replace('(','').replace(')','')+esp+chaine
    return chaine #chaine.encode(codec)
...
#    nchaine = unicode(chaine.decode('latin-1'))
    ...
    sols = re.findall(ur'(suspension|solution)(injectable|perfusion|reconstituée|acides aminés|prête)?', nchaine,re.UNICODE)
    if len(sols)>0:
        return ('solution injectable', True)
    sols = re.findall(ur'comprimé|gélule|capsule', nchaine,re.UNICODE)
    if len(sols) > 0:
...
def isVaccin(word1,word2,codec='latin-1'):
    sol1=re.findall(ur'VIRUS|ATTÉNUÉ|SOUCHE|NEISSERIA MENINGITIDIS|PNEUMOCOCCIQUE', word1,re.UNICODE)  #sol1=re.findall(ur'VIRUS|ATTÉNUÉ|SOUCHE|NEISSERIA MENINGITIDIS|PNEUMOCOCCIQUE', word1.decode(codec))
    sol2=re.findall(ur'vaccin', word2,re.UNICODE)  #sol2=re.findall(ur'vaccin', word2.decode(codec))
...
def GetComposition(word1,word2):
    ...
    try:
        ...
        valeur1=-1.0
    unite=re.findall(ur'\S+',word1,re.UNICODE)
    if len(unite)==0:
...
#    unite=unicode(unite.decode('latin-1'))
    unite=re.sub(ur'[Mm]illigrammes?','mg',unite,re.UNICODE)
    unite=re.sub(ur'[Mm]icrogrammes?','µg',unite,re.UNICODE)
    unite=re.sub(ur'[Nn]anogrammes?','mg',unite,re.UNICODE)
    unite=re.sub(ur'[Gg]rammes?','g',unite,re.UNICODE)
    unite=re.sub(ur'[Ll]itres?','l',unite,re.UNICODE)
    unite=re.sub(ur'[Mm]illilitres?','ml',unite,re.UNICODE)
    unite=re.sub(ur'[Mm]icrolitres?','µl',unite,re.UNICODE)
    unite=re.sub(ur'[mM]illions? d\'unités? internationales?|millions UI','MUI',unite,re.UNICODE)
    unite1=unite
...
if __name__ == '__main__':
    opt=input(u"Voulez-vous vraiment rafraichir la base médicaments (O/n)?:")
    if not opt=='O':
        sys.exit(1) 
    t0=time.time()
